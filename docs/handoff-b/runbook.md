...
缺失项可以使用 Main 中预设的官方页面。系统 WinGet 可用时，也可以在确认后打开可见 PowerShell 安装终端；软件包 ID 和命令由 Main 固定，不接受 Renderer 自定义。
...
完整 E2E 覆盖安全 Preload、Monaco 保存、编译运行、真实 GDB 变量单步、CMake/CTest、响应式布局和深色主题。
