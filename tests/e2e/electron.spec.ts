...
test('launches securely and renders the real project workflow', async () => {
  ...
  try {
    ...
    await dragSeparator('调整快照侧边栏宽度', -36, 0)
    ...
    const snapshots = page.locator('.snapshot-list article')
    const saveCurrentEdit = async (previousSnapshotCount: number) => {
      const saveButton = page.getByRole('button', { name: '保存', exact: true })
      await expect.poll(async () => (await saveButton.isEnabled()) || (await snapshots.count()) > previousSnapshotCount).toBe(true)
      if (await saveButton.isEnabled()) await saveButton.click()
      await expect.poll(async () => snapshots.count()).toBeGreaterThan(previousSnapshotCount)
    }
    await expect(editor).toBeVisible()
    const firstSnapshotCount = await snapshots.count()
    await editor.click()
    ...
    if (toolchain.ok && toolchain.data?.family === 'gcc' && toolchain.data.debuggerPath) {
      const debugSnapshotCount = await snapshots.count()
      await editor.click()
      await page.keyboard.press('Control+A')
