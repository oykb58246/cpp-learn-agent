...
export const builtInKnowledge: KnowledgeNode[] = [
  ...
  node('math.prime', '素数与整除', 'math', 2, ['control.loops'], ['prime', 'divisibility']),
  node('math.gcd', '最大公约数', 'math', 2, ['functions.basic', 'control.loops'], ['gcd', 'lcm']),
  node('recursion.basic', '递归', 'functions', 2, ['functions.basic', 'control.conditions'], ['recursion']),
  node('sorting.basic', '排序', 'algorithm', 2, ['data.arrays', 'stl.algorithm'], ['sort', 'comparator']),
  node('search.binary', '二分查找', 'algorithm', 3, ['sorting.basic'], ['binary-search', 'lower_bound']),
  node('stl.set', 'set 容器', 'stl', 3, ['generic.templates', 'stl.iterators'], ['set']),
  node('stl.queue', 'queue 队列', 'stl', 2, ['generic.templates', 'data.structs'], ['queue']),
  node('stl.stack', 'stack 栈', 'stl', 2, ['generic.templates', 'data.structs'], ['stack']),
