...
const patterns = {
  edit: /改成|替换|修改|编译成功|写入|添加|删除|插入|replace|change|edit|modify/i,
...
export function inferAgentMode(input: IntentInput): AgentStartRequest['mode'] {
  ...
  const explanationLead = /^(?:请|帮我|能否|可以)?\s*(?:解释|讲解|说明)/.test(message)
  const laterExecution = /(?:并|然后|再)(?:帮我)?\s*(?:修改|替换|改成|改为|写入|添加|删除|插入)/.test(message)
  if (explanationLead && !laterExecution) return 'explain'
  if (patterns.edit.test(message)) return 'edit'
