...
export function environmentInstallOutcome(task: EnvironmentInstallTask): { message: string; redetect: boolean } {
  if (task.status === 'running') {
    return { message: `${labels[task.target]}安装终端正在运行。`, redetect: false }
  }
  ...
  if (task.status === 'succeeded') {
    return { message: `${labels[task.target]}安装完成，正在自动重新检测环境…`, redetect: true }
  }
  const exit = task.exitCode === null || task.exitCode === undefined ? '' : `（退出码 ${task.exitCode}）`
  return { message: `${labels[task.target]}安装未成功${exit}，可以重试或改用官网安装。`, redetect: false }
}
