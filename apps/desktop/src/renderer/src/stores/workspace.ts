...
export interface OpenTab extends FileDocument { draft: string; dirty: boolean; conflicted: boolean }
export const useWorkspaceStore = defineStore('workspace', {
  ...
  actions: {
    ...
    async openProject(id: string) { this.loading = true; const result = await window.cppPet.project.open({ projectId: id }); if (result.ok) { this.currentProject = result.data; await Promise.all([this.refreshTree(), this.refreshSnapshots()]) } else this.error = result.error; this.loading = false },
    ...
    async save() {
      const tab = this.activeTab; if (!tab || !tab.dirty) return
      this.saving = true
      ...
      if (result.ok) Object.assign(tab, result.data, { draft: result.data.content, dirty: false, conflicted: false })
    ...
    handleExternal(event: { projectId?: string; relativePath?: string }) { if (event.projectId !== this.currentProject?.id) return; void this.refreshTree(); const tab = this.tabs.find(x => x.relativePath === event.relativePath); if (tab) tab.dirty ? tab.conflicted = true : void this.reloadTab(tab.relativePath) },
    async reloadTab(path: string) { const tab = this.tabs.find(x => x.relativePath === path); if (!tab || !this.currentProject) return; const result = await window.cppPet.files.read({ projectId: this.currentProject.id, relativePath: path }); if (result.ok) Object.assign(tab, result.data, { draft: result.data.content, dirty: false, conflicted: false }) }
  }
})
